...
func TestRateAdd(t *testing.T) {
	t.Run("add single frequency", func(t *testing.T) {
	...
	t.Run("add multiple frequencies", func(t *testing.T) {
		r := NewRate(60 * time.Second)
		now := time.Now()
	...
	t.Run("add with decay over time", func(t *testing.T) {
		...
		r.AddAt(20.0, now.Add(60*time.Second))
		rate2 := r.RateAt(now.Add(60 * time.Second))

		// rate2 should be: 100 * 0.5 + 20 = 70
		assert.Equal(t, 100.0, rate1)
		assert.InDelta(t, 70.0, rate2, 0.01)
	})
}
...
func TestRateDecay(t *testing.T) {
	...
	t.Run("short half-life decays faster", func(t *testing.T) {
		...
		rShort.AddAt(100.0, now)
		rLong.AddAt(100.0, now)
	...
	t.Run("rate approaches zero over many half-lives", func(t *testing.T) {
		...
		// Should be very close to zero (1000 * 0.5^10 ≈ 0.977)
...
func TestRateQuery(t *testing.T) {
	t.Run("rate returns current rate with automatic decay", func(t *testing.T) {
		...
		rate := r.Rate()
		// Should be slightly less than 100 due to decay
		assert.Less(t, rate, 100.0)
		assert.Greater(t, rate, 99.9)
	})
...
func TestRateEdgeCases(t *testing.T) {
	t.Run("add zero", func(t *testing.T) {
		...
		assert.Equal(t, 100.0, r.RateAt(now))
	})
	...
	t.Run("add negative", func(t *testing.T) {
		...
		assert.Equal(t, 70.0, r.RateAt(now))
	})
	...
	t.Run("add at earlier time uses last rate", func(t *testing.T) {
		...
		r.AddAt(100.0, now)
		r.AddAt(50.0, now.Add(-10*time.Second))

		// Should just add without decay
		assert.Equal(t, 150.0, r.RateAt(now))
	})
}

func TestRateRealisticScenario(t *testing.T) {
	t.Run("simulate frequency rate tracking", func(t *testing.T) {
		r := NewRate(60 * time.Second)
		now := time.Now()

		// Simulate frequency counts over time
		// 0s: 100 requests
		r.AddAt(100.0, now)

		// 30s: 50 requests (100 * sqrt(0.5) + 50 ≈ 120.7)
		r.AddAt(50.0, now.Add(30*time.Second))
		rate30 := r.RateAt(now.Add(30 * time.Second))

		// 60s: 80 requests (120.7 * 0.5 + 80 ≈ 140.35)
		r.AddAt(80.0, now.Add(60*time.Second))
		rate60 := r.RateAt(now.Add(60 * time.Second))

		// Rates should be reasonable
		assert.Greater(t, rate30, 50.0)   // More than just new count
		assert.Less(t, rate30, 150.0)     // Less than sum due to decay
		assert.Greater(t, rate60, 80.0)   // More than just new count
